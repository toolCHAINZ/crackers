...
def install_with_apt():
    print(
        "Detected apt. Installing llvm-dev, libclang-dev, and clang...", file=sys.stderr
    )
    try:
        subprocess.run(["apt", "update"], check=True)
        subprocess.run(
            ["apt", "install", "-y", "build-essential", "libc6-dev", "gcc-multilib"],
            check=True,
        )
...
def install_z3_glibc(target_platform):
    print(
        f"Fetching and installing Z3 Linux glibc build for target platform '{target_platform}'...",
        file=sys.stderr,
    )
    ...
    if not glibc_url:
        raise Exception(
            f"Could not find a suitable Z3 glibc build for platform {target_platform}."
        )
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        subprocess.run(
            ["cp", os.path.join(lib_dir, "libz3.so"), "/usr/local/lib/"], check=True
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Install Z3 glibc build for a target platform."
    )
    parser.add_argument(
        "target_platform",
